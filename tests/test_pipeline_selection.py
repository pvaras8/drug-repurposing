...
def test_select_boltz_candidates_can_send_every_completed_molecule() -> None:
    scores = [(f"mol_{index}", float(-index)) for index in range(1, 9)]

    selected, threshold = _select_boltz_candidates(
        scores, max_molecules=None, quantile=1.0
    )

    assert selected == {f"mol_{index}" for index in range(1, 9)}
